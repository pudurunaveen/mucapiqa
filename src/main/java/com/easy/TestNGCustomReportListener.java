...
public class TestNGCustomReportListener implements IReporter{
	...
	private void resultSummary(ISuite suite, IResultMap tests, String testname,
		...
		if (tests.getAllResults().size() > 0) {
			...
			Collection<ITestNGMethod> ngList = getMethodSet(tests, suite);
			
	        Iterator<ITestNGMethod> iterator = ngList.iterator();
	        
	        // while loop
	        while (iterator.hasNext()) {
	        	ITestNGMethod testNGmethodd =  iterator.next();
	        	ITestClass testClass = testNGmethodd.getTestClass();
	        //}
			
			//for (ITestNGMethod testNGmethodd : ngList) {
				m_row += 1;
				m_methodIndex += 1;
				...
				Set<ITestResult> resultSet = tests.getResults(testNGmethodd);
				long end = Long.MIN_VALUE;
				long start = Long.MAX_VALUE;
				long startMS=0;
				String firstLine="";
				
				for (ITestResult testResult : tests.getResults(testNGmethodd)) {
